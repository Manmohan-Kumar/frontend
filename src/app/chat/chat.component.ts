...
export class ChatComponent implements OnInit {
  ...
  getChatHistory(name) {
      const receiver_id = this.contact.user_id == undefined?'1':this.contact.user_id;
  ...
  sendMessage(message,contact){
    ...
    const receiver_id = contact.user_id;
    ...
    const recentChat = new Chat(message.value,this.sender_id, receiver_id, '0', new Date());
    message.value = '';
